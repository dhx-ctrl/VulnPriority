...
function SyncPage() {
  ...
  const handleFetchProducts = async () => {
    ...
    setProductsError('');
    try {
      const list = await window.ApiClient.getProducts();
      setProducts(list);
      if (list.length === 0) {
  ...
  const handleSync = async () => {
    ...
    if (window.USE_MOCK_DATA) {
      ...
      const advance = () => {
        ...
        if (step < steps.length - 1) {
          ...
          setSyncState('done');
          setResult(window.MOCK_SYNC_STATUS);
    ...
    try {
      ...
      setStepIndex(1);
      const data = await window.ApiClient.syncDefectDojo(payload);
      ...
      setStepIndex(4);
      await window.ApiClient.refreshCache();
  ...
  const sev = useMemo(() => {
    ...
    const src = window._cachedFindings;
    if (!src || src.length === 0) return window.MOCK_SEVERITY_BREAKDOWN;
  ...
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 20 } },
    React.createElement('div', null,
      ...
      React.createElement('p', { style: { fontSize: 14, color: sub, margin: 0 } }, 'Sync findings from DefectDojo and run AI scoring'),
    ),
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '380px 1fr', gap: 20, alignItems: 'start' } },
      ...
      React.createElement('div', null,
        result && React.createElement(GlassCard, null,
          ...
          React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 } },
            [
              ...
              { label: 'High-Risk Flagged', val: result.high_risk_flagged, highlight: true },
          ...
          }, 'Dashboard, Findings and Scan History have been refreshed with the new data.'),
        ),
        // Idle
        syncState === 'idle' && !result && React.createElement(GlassCard, {
          ...
          React.createElement('div', { style: { fontSize: 14, color: muted, marginTop: 12 } }, 'Configure and run a sync to see results'),
        ),
        // Error
