...
function ModelInsightsPage() {
  ...
  const cleanName =
    cleanModel.model || "XGBoost leakage-safe binary classifier";
  ...
  const ablationData = [
    ...
    { test: "no source", aucpr: 0.74, roc: 0.897 },
  ...
  return (
    ...
    >
      <div>
        ...
        >
          The dashboard uses two XGBoost models: an operational EPSS ranker for
          sorting the review queue, and a clean leakage-safe model as a stricter
          confidence signal.
        </p>
      </div>
      <div
        ...
        <GlassCard>
          ...
          >
            <div>
              ...
              >{`${rankerName} · ${rankerFeatureCount} features`}</div>
            </div>
            <MiniBadge color="#3884f4">main sorting model</MiniBadge>
          </div>
          <p
            style={{
              fontSize: 13,
          ...
          >
            This model is used to order the analyst review queue. It was trained
            with EPSS-only labels, so CVSS can be used as an input feature
            without circularly copying the label.
          </p>
          <MetricRow
            label="Test AUCPR"
            value="0.265"
        ...
        <GlassCard>
          ...
          >
            <div>
              ...
              >{`${cleanName} · ${cleanFeatureCount} minimal features`}</div>
            </div>
            <MiniBadge color="#22c55e">strict confidence signal</MiniBadge>
          </div>
          <p
            style={{
              fontSize: 13,
          ...
          >
            This model is kept as the scientifically defensible baseline. EPSS,
            CVSS score, severity, raw advisory IDs, source metadata, exploit
            references, and CVSS subcomponents were removed from its inputs.
          </p>
          <MetricRow
            label="Precision"
            value="0.772"
          ...
          />
          <MetricRow label="F1-score" value="0.620" />
          <MetricRow label="AUCPR" value="0.717" />
          <MetricRow label="ROC-AUC" value="0.888" />
          <MetricRow
      ...
      >
        <GlassCard>
          ...
          >
            <InfoBox title="Rank /100" color="#3884f4">
              The operational EPSS ranker score. This is the main score used to
              sort the review queue. A higher score means the finding should be
              reviewed earlier.
            </InfoBox>
            <InfoBox title="Clean /100" color="#22c55e">
              The clean leakage-safe model score. It is shown as a secondary
              confidence signal and is not used to hide lower-scoring findings.
            </InfoBox>
            <InfoBox title="Scanner Severity" color="#f97316">
              The original severity from DefectDojo or the scanner. It remains
              visible because scanner severity and AI priority answer different
              questions.
            </InfoBox>
            <InfoBox title="Priority label" color="#e0364c">
              Review First means the operational rank score is high enough for
              immediate queue priority. Review Soon and Severity Watch keep
              important findings visible without pretending every scanner-high
              item is an AI emergency.
            </InfoBox>
          </div>
        </GlassCard>
        <GlassCard>
          ...
          >
            On the held-out EPSS ranking test, the AI ranker outperformed CVSS
            on AUCPR, ROC-AUC, and NDCG. It also required fewer findings to be
            reviewed to cover 80% of EPSS-positive cases.
          </p>
        </GlassCard>
      </div>
      <div
      ...
      >
        <GlassCard>
          ...
          >
            The clean model is strict: it keeps false positives low, but misses
            some true positives. For that reason, the dashboard does not use it
            as a hard gate.
          </p>
        </GlassCard>
        <GlassCard>
          ...
          >
            Removing source metadata and CVSS subcomponents did not collapse the
            clean model, which supports that it is not only copying source bias
            or CVSS logic.
          </p>
        </GlassCard>
      </div>
      <GlassCard>
        ...
        >
          The AI score is not a vulnerability detector and does not replace
          SAST, SCA, DAST, CVSS, or analyst review. Scanner findings remain the
          source of detection. The AI models only help prioritize which findings
