...
function normalizeRiskCategory(f) {
  const raw = String(f.risk_category || "")
    .replace(/\s*Risk$/i, "")
    .trim();
...
function scannerSeverityRank(f) {
  const s = String(
    f.severity || f.scanner_severity || f.defectdojo_severity || "",
  )
    .trim()
    .toLowerCase();
...
function DashboardPage() {
  ...
  const donutData = Object.entries(sev)
  ...
  const barData = useMemo(() => {
    ...
    findings.forEach((f) => {
      ...
      const s =
        ...
        "Medium";
      if (!byProduct[p])
        byProduct[p] = { name: p, Critical: 0, High: 0, Medium: 0, Low: 0 };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      >
        <div>
          ...
          >
            ...
            >
              Live local DevSecOps dashboard
            </span>
          </div>
          <h1
            style={{
          ...
          <p style={{ fontSize: 14, color: c.sub, margin: "6px 0 0" }}>
            AI-prioritized vulnerability overview from DefectDojo, Semgrep,
            Trivy, and ZAP.
          </p>
        </div>
        <div
      ...
      >
        <GlassCard delay={80} style={{ minHeight: 330 }}>
          <SectionTitle
            title="Scanner Severity Distribution"
            subtitle="Original scanner / DefectDojo severity. This is separate from AI priority."
          />
          {donutData.length === 0 ? (
            <EmptyPanel text="No severity data yet. Sync a DefectDojo product first." />
          ) : (
        ...
        <GlassCard delay={120} style={{ minHeight: 330 }}>
          <SectionTitle
            title="Findings by Product"
            subtitle="Real synced product names, grouped dynamically."
          />
          {barData.length === 0 ? (
            <EmptyPanel text="No product data yet. Use the Sync page to fetch findings." />
          ) : (
            <ResponsiveContainer width="100%" height={235}>
              <BarChart
                data={barData}
              ...
              >
                <CartesianGrid strokeDasharray="3 3" stroke={c.softBorder} />
                <XAxis
                  dataKey="name"
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "minmax(420px, 1.2fr) minmax(300px, 0.8fr)",
          gap: 18,
        }}
      >
        <GlassCard delay={160} style={{ minHeight: 330 }}>
          <SectionTitle
            title="Findings Trend"
            subtitle="Weekly severity movement from backend trend endpoint."
          />
          {trends.length === 0 ? (
            <EmptyPanel text="No trend data available yet." />
          ) : (
            <ResponsiveContainer width="100%" height={235}>
              <AreaChart
                data={trends}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div
              style={{
                ...
                border: `1px solid ${dark ? "rgba(79,125,243,0.22)" : "rgba(79,125,243,0.14)"}`,
              }}
            >
              <div
                style={{
            ...
            <div
              style={{
                ...
                borderRadius: 12,
                background: dark
                  ? "rgba(155,107,255,0.10)"
                  : "rgba(155,107,255,0.06)",
                border: `1px solid ${dark ? "rgba(155,107,255,0.22)" : "rgba(155,107,255,0.14)"}`,
              }}
            >
              <div
                style={{
      ...
      <GlassCard delay={240}>
        <SectionTitle
          ...
          right={
            ...
            >{`${fixNow.length} items`}</span>
          }
        />
        {fixNow.length === 0 ? (
          <EmptyPanel text="No findings yet. Run a product sync to populate the priority list." />
        ) : (
          <div style={{ overflowX: "auto" }}>
            <div
              style={{
                display: "grid",
                gridTemplateColumns:
                  "130px minmax(260px,1fr) 104px 80px 88px 72px",
                gap: 12,
                padding: "0 16px 10px",
                color: c.muted,
                fontSize: 10,
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      >
                        ...
                        <span
                          style={{ color: sevColor, fontWeight: 800 }}
                        >{`Scanner severity: ${severity}`}</span>
                      </div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <PriorityPill tier={dashboardPriorityTier(f)} />
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <ScannerPill scanner={f.scanner_type || "SCA"} />
                    </div>
